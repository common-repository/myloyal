    ...
    public function run( $hook, $rules_array ) {
	    add_action( $hook, function ( $comment_id, $comment ) use ( $hook, $rules_array ) {
		    ...
		    foreach ( $rules_array as $k => $rule ) {
			    ...
			    switch ( $rule['case'] ) {
				    case 'when_user_gets_comment':
					    Action_Runner::instance()->set_point( $rule, $point, $parent_post->post_author );
				    	break;
				    case 'when_user_creates_comment':
				    	if ( isset( $commenter->ID ) ) {
